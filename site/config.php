...
function check_for_csrf() {
  ...
  if(strpos($_SERVER['PHP_SELF'], "ajax/edit") || strpos($_SERVER['PHP_SELF'], "ajax/add")) {
    ...
    $query = "SELECT `users_rights_edit` FROM `users_rights` WHERE `user_id` = '".$_SESSION['customer_id']."' AND SHA1( menu_id ) = '$user_access'";
    //echo $query;exit;
  ...
  if(strpos($_SERVER['PHP_SELF'], "ajax/delete")) {
    ...
    $query = "SELECT `users_rights_delete` FROM `users_rights` WHERE `user_id` = '".$_SESSION['customer_id']."' AND SHA1( menu_id ) = '$user_access'";
    $result = mysqli_query($db_link, $query);
    if(!$result) {
