...
?>
  <div id="order-detail-content" class="table_block table-responsive">
    <table id="cart_summary" class="table table-bordered stock-management-on">
      <thead>
        <tr>
          <th class="item" width="15%"><?=$languages['header_product'];?></th>
          <th class="item" width="60%"><?=$languages['header_description'];?></th>
          <th class="item" width="15%"><?=$languages['header_unit_price'];?></th>
          <th class="item" width="10%"><?=$languages['header_delete'];?></th>
        </tr>
      </thead>
      <tbody>
<?php
...
<?php
    ...
    echo '<p class="alert alert-warning">'.$languages['text_wishlist_is_empty'].'</p>';;
  }
?>
  <p class="clearfix">&nbsp;</p>
  ...
  <div style="display:none;" id="modal_confirm" class="clearfix" title="<?=$languages['are_you_sure'];?>">
    <p style="padding:0;margin:0;width:100%;float:left;"><?=$languages['delete_product_from_wishlist'];?></p>
  </div>
  <script>
  $(function() {
