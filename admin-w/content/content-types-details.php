<?php
  ...
  if(isset($_POST['submit_content_type'])) {
...
//    echo"<pre>";print_r($_POST);print_r($_FILES);exit;
    ...
    if(empty($content_type_errors)) {
      ...
      $query_update = "UPDATE `contents_types` SET `content_type` = '$content_type',`content_type_is_active` = '$content_type_is_active' WHERE `content_type_id` = '$current_content_type_id'";
      //echo $query_update;
      ...
      foreach($content_type_names_array as $language_id => $content_type_name) {
        ...
        if(isset($_POST['new_entry'][$language_id])) {
          ...
          $query_content_type = "INSERT INTO `contents_types_languages`(`content_type_id`,`language_id`,`content_type_name`) 
                                                                VALUES ('$current_content_type_id','$language_id','$content_type_name')";
          ...
          $query_update_content_type = "UPDATE `contents_types_languages` SET `content_type_name` = '$content_type_name'
                                          WHERE `content_type_id` = '$current_content_type_id' AND `language_id` = '$language_id'";
          //echo $query_update_content_type;
          $all_queries .= "<br>".$query_update_content_type;
          $result_update_content_type = mysqli_query($db_link, $query_update_content_type);
          if(!$result_update_content_type) {
            echo $languages['sql_error_update']." 3 contents_types_languages - update ".mysqli_error($db_link);
            mysqli_query($db_link,"ROLLBACK");
            exit;
          }
        }
    ...
    $query_content_type = "SELECT `content_type`,`content_type_is_active` FROM `contents_types` WHERE `content_type_id` = '$current_content_type_id'";
    //echo $query_content_type;
    $result_content_type = mysqli_query($db_link, $query_content_type);
    if(!$result_content_type) echo mysqli_error($db_link);
    if(mysqli_num_rows($result_content_type) > 0) {
      $content_type_row = mysqli_fetch_assoc($result_content_type);
      $content_type = $content_type_row['content_type'];
      $content_type_is_active = $content_type_row['content_type_is_active'];
    }
  }
...
<?php if(isset($content_type_errors) && !empty($content_type_errors)) echo '<div class="warning">Моля проверете дали всички задължителни полета са попълнени</div>';?>
...
<?php
        if(!empty($languages_array)) {
          ...
          foreach($languages_array as $row_languages) {
            ...
            if(!isset($_POST['submit_content_type'])) {
              $query_content_type = "SELECT `content_type_name` FROM `contents_types_languages` 
                                      WHERE `content_type_id` = '$current_content_type_id' AND `language_id` = '$language_id'";
              //echo $query_content_type;
              $result_content_type = mysqli_query($db_link, $query_content_type);
              if(!$result_content_type) echo mysqli_error($db_link);
              if(mysqli_num_rows($result_content_type) > 0) {
                $content_type_array = mysqli_fetch_assoc($result_content_type);
                //echo"<pre>";print_r($attribute_group_array);
                $content_type_names_array[$language_id] = $content_type_array['content_type_name'];
              }
            }
?>
          <div class="col-lg-6 col-md-8 col-sm-12 col-xs-12">
...
?>
        ...
        <div>
          <button type="submit" name="submit_content_type" class="button green"><i class="fa fa-floppy-o" aria-hidden="true"></i> <?=$languages['btn_save'];?></button>
