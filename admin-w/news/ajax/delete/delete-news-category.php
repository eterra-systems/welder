...
?>
  <div class="warning" style="margin-top: 10px;">
    <?=$languages['text_delete_news_first'];?>
...
<?php
  ...
  if($news_cat_parent_id != 0) {
    $query_news_categories_siblings = "SELECT `news_category_id` FROM `news_categories` WHERE `news_cat_parent_id` = '$news_cat_parent_id' LIMIT 1";
    ...
    if(mysqli_num_rows($result_news_categories_siblings) <= 0) {
      ...
      if(!$result_update_parent) {
        echo $languages['sql_error_update']." - 4 ".mysqli_error($db_link);
        mysqli_query($db_link,"ROLLBACK");
        exit;
      }
