    ...
    */
    ...
    if($current_news_cat_parent_id != $news_cat_parent_id) {
      ...
      if($news_cat_parent_id == 0) {
        ...
        if(!$result_update_parent) {
          echo $languages['sql_error_update']." - 1 UPDATE `news_categories` ".mysqli_error($db_link);
    ...
    if(!isset($news_category_errors)) {
      ...
      if(!$result_update_news_category) {
        echo $languages['sql_error_update']." - 2 UPDATE `news_categories` ".mysqli_error($db_link);
        mysqli_query($db_link,"ROLLBACK");
        exit;
      }
      ...
      if($current_news_cat_parent_id != 0 && $current_news_cat_parent_id != $news_cat_parent_id) {
        ...
        if(mysqli_num_rows($result_news_categories_siblings) <= 0) {
          ...
          $result_update_parent = mysqli_query($db_link, $query_update_parent);
          if(!$result_update_parent) {
            echo $languages['sql_error_update']." - 3 UPDATE `news_categories` ".mysqli_error($db_link);
            mysqli_query($db_link,"ROLLBACK");
            exit;
          }
        ...
        if(mysqli_num_rows($result_news_categories_for_reorder) > 0) {
          while($row_news_categories_for_reorder = mysqli_fetch_assoc($result_news_categories_for_reorder)) {
            ...
            if(!$result_update_category) {
              echo $languages['sql_error_update']." - 4 UPDATE `news_categories` ".mysqli_error($db_link);
              mysqli_query($db_link,"ROLLBACK");
              exit;
            }
          }
      ...
      foreach($news_cat_names as $language_id => $news_cat_name) {
        ...
        if(empty($current_news_cat_name)) {
          ...
          if(mysqli_affected_rows($db_link) <= 0) {
            echo $languages['sql_error_insert']." - 5 INSERT INTO `news_cat_desc` ".mysqli_error($db_link);
            mysqli_query($db_link,"ROLLBACK");
            exit;
          }
        }
        else {
          if($current_news_cat_name != $news_cat_name || $current_news_cat_parent_id != $news_cat_parent_id) {
            //update the name
            ...
            if(!$result_update_news_cat_desc) {
              echo $languages['sql_error_update']." - 6 UPDATE `news_cat_desc` ".mysqli_error($db_link);
              mysqli_query($db_link,"ROLLBACK");
              exit;
            }
...
?>
        <div>
          <p class="title"><?=$languages['header_news_category_parent'];?><span class="red">*</span></p>
...
<?php
            list_news_categories_for_select($parent_id = 0, $path_number = 0, $current_news_category_parent_id = $news_cat_parent_id, $current_news_category_id);
?> 
          </select>
        </div>
        ...
        <div>
          <button type="submit" name="update_news_category" class="button green"><i class="fa fa-floppy-o" aria-hidden="true"></i> <?=$languages['btn_save'];?></button>
